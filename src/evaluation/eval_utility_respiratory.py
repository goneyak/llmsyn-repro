...
if __name__ == "__main__":
    ...
    for seed in seeds:
        ...
        X_real_sub = X_real[train_idx]
        y_real_sub = y_real[train_idx]
        X_test_real = X_real[test_idx]
        y_test_real = y_real[test_idx]

        rows.append(run_rf("REAL_train_REAL_test", X_real_sub, y_real_sub, X_test_real, y_test_real, seed))
        ...
        for name, (x_path, y_path) in syn_files.items():
            ...
            rows.append(run_rf(f"FS_{name}_train_SYN_test_REAL", X_syn, y_syn, X_test_real, y_test_real, seed))
            X_da = np.vstack([X_real_sub, X_syn])
            y_da = np.concatenate([y_real_sub, y_syn])
